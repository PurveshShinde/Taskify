                  ...
                  {recentProjects.map(project => (
                     <div key={project._id} className="bg-white dark:bg-dark-800 p-6 rounded-xl border border-light-200 dark:border-dark-600 shadow-sm flex justify-between items-center hover:shadow-md transition">
                        ...
                        <div className="flex flex-col items-end">
                           <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${project.status === 'active' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-slate-100 text-slate-600'
                              }`}>
                              {project.status}
                           </span>
