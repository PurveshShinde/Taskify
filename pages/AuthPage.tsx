...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        const { user, token } = await api.auth.register(registerName, registerEmail, registerPassword);
        // Note: Usually registration requires email verification first, but if token returned, we login
        if (token) login(token, user);
        alert("Account created! Please check your email to verify before logging in.");
        navigate('/auth?mode=signin'); // Redirect to login after signup
      } else {
        const { user, token } = await api.auth.login(loginEmail, loginPassword);
        login(token, user);
